...
struct XVEdge1D
{
    ...
    XVPoint2D               point       ;        //坐标
    float                   magnitude   = 0.0f;  //梯度幅值
    XVEdgeTransitionType    transition  = XVEdgeTransitionType::Any; //方向
};
...
struct XVStripeScanParams
{
    ...
    XVFloat			    maxInnerEdgeMagnitude              ;        //NUL   Maximum strength of edges appearing between the two ends of a stripe(条内部最大梯度不超过该值 默认值0时该参数不起作用)！！！！
    ...
    XVFloat                         maxStripeWidth                     ;        //NUL   Maximum acceptable stripe width in pixels
};
...
static void CopyArray_XVEdge1D(const XVArray_XVEdge1D& src, XVArray_XVEdge1D& dest)
{
...
static void CopyArray_XVRidge1D(const XVArray_XVRidge1D& src, XVArray_XVRidge1D& dest)
{
    dest.optional = src.optional;
    dest.array.clear();
    for(auto c : src.array)
...
static void CopyArray_XVStripe1D(const XVArray_XVStripe1D& src, XVArray_XVStripe1D& dest)
{
    dest.optional = src.optional;
    dest.array.clear();
    for(auto c : src.array)
...
static void CopyArray_XVGap1D(const XVArray_XVGap1D& src, XVArray_XVGap1D& dest)
{
    dest.optional = src.optional;
    dest.array.clear();
    for(auto c : src.array)
